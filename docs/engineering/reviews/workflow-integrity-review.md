...
| F2 — بوابة الأنواع في pre-push | **منفَّذ بالبديل (دفعة بوابة الجودة)**: الـ hook يترجم الـ API إلى `dist-openapi/` بشكل incremental ويفحص `openapi.json` في 11–19 ث، وبقية الفحوص في CI | `docs/engineering/quality-gates.md` |
...
| اختبار `[H2]` | **صار حتميًا (دفعة CI أخضر)**: الإرسالان المتوازيان كانا يعتمدان على التوقيت، ونجحا مرة في 2026-09-11 فأفشل Jest الـ `it.failing`. الآن `holdConcurrentReads` في الاختبار يمسك جواب `findActive` حتى تنتهي القراءتان، ويبقى `it.failing` حتى F5 | 10 تكرارات متتالية: الاثنتان نشطتان في كل مرة (متوقَّع 1، الناتج 2) |

...
| البند | الحالة |
|---|---|
| المرحلة أ (تصميم، قراءة فقط) | **مكتملة 2026-09-12.** أ١–أ٦ والأسئلة س١–س١٧ معتمدة |
| المرحلة ب (الـ backend) | **مكتملة.** وحدات ٨٢٢، e2e ٧٨ |
| المرحلة ج (الداش بورد) | **مكتملة بانتظار جولة المالك الحيّة الأخيرة (2026-09-13).** الخطوات ج٠–ج٩ ودفعة الإغلاق معتمدة. suite الداش بورد ٨١٧ اختبارًا في ٥٨ ملفًا، والسيرفرات متوقفة |
| المرحلة د (الصفحة العامة) | لم تبدأ. لا وجود لصفحة كلمة الرئيس في `apps/web` بعد — رابط في التنقل فقط |
| المرحلة هـ (التحقق الحيّ) | لم تبدأ |
| البنود المعلّقة | ثلاثة — انظر «البنود المعلّقة عند إغلاق المرحلة ج» في آخر الملف |
...
> **مُستبدَلة بـ ADR-0069 D10 (2026-09-13).** الجدول أدناه هو المسودة الأولى، وبقي للتاريخ فقط. السيناريو المعتمد في D10: لا opacity على أي محتوى، وخطوط الهوية بدل الـ motif، وصعود الصورة 16px بدل الـ clip، وظهور مرة واحدة بدل `rise-scroll`، والفقرات ثابتة. عند أي تعارض يحكم D10.
...
### ج٤: محرر TipTap (الاعتماديات معتمَدة، س٦) — **تم**
...
### ج٥: أقسام التحرير الستة والصفحة والتنقل — **تم**
...
### ج٦: لوحة الحالة والموافقات العامة — **تم**
...
### ج٧: الإصدارات والاسترجاع العامان (بلا مقارنة كلمة بكلمة) — **تم**
...
- **الاسترجاع (قرار المالك 2026-09-12):** الخطر في التعديلات غير المحفوظة لا في الاسترجاع نفسه، فيُزال الخطر بدل رفع التحذير:
  ...
  2. بعد أن تصبح الصفحة نظيفة، الاسترجاع يمر **بـ`ConfirmDialog`** (قرار المالك 2026-09-13؛ كان لوحة داخل الصفحة — انظر «الاسترجاع: لوحة في الصفحة أم `ConfirmDialog`» أدناه) يوضح: رقم النسخة وتاريخها، وأن المحتوى الحالي سيُستبدل، وأن المسترجع يعود كمسودة فيمر بالنشر العادي.
...
### ج٨: ترحيل الشاشات الثلاث إلى الـ toasts — **تم**
...
### قرارات المالك أثناء د١ — 2026-09-13

| القرار | ما يعنيه | ما يتطلبه |
|---|---|---|
| **الاتجاه المختار للـ hero: ج** — بشرط خلوّه من المشاكل والأخطاء ومطابقته لمعايير UI/UX | صورة أرضية تحت `HERO_SCRIM`، والمكتب على حافة النطاق السفلية. التركيب يُشتق من البيانات كما في ADR-0067 D2: مع `heroImage` يظهر ج، وبدونه يظهر النطاق الأخضر نفسه (الاتجاه ب). السجل المنشور اليوم **بلا** صورة خلفية، والأرضية في صفحة المراجعة بديلة من تصدير Figma (`2705:4`) | يُتحقَّق من الشرط (نقد impeccable بتقييميه، والتباين فوق الـ scrim، وLCP، والاتجاهان، والموبايل، والغامق) **قبل** بناء د٢. لكي يظهر ج فعلًا على الموقع يرفع محرر صورة خلفية للسجل وينشرها |
| **الصورة يمينًا والنص يسارًا في العربي** (والإنجليزي كما هو: النص يسارًا والصورة يمينًا) | الـ portrait hero يضع الصورة في جهة ثابتة لا تتبع اتجاه القراءة. يتعارض مع §7.4 (صف تركيب الـ hero) ومع ADR-0066 D5 («كتلة العنوان على جهة القراءة»). الحجة التي تسنده: جسد الرئيس في الصورة متجه نحو يسار الإطار، فعلى اليمين يواجه النص في اللغتين، والصورة لا تُعكس لأنها صورة شخص حقيقي | **تعديل موثَّق** على §7.4 وADR-0069 D8 للـ portrait hero وحده، يُكتب عند اعتماد الاتجاه في د٢. تحت `lg` يبقى العنوان أولًا ثم الصورة (§5.10) |
| **الـ hero بارتفاع الشاشة ناقص الهيدر، ويظهر كاملًا** | في صفحة كلمة الرئيس الآن: ارتفاع ثابت `100svh − var(--space-24)`، والصورة تنكمش في المساحة التي يتركها العنوان ولا تتجاوز حجمها الطبيعي | — |
| **القاعدة نفسها لبقية heroes الموقع** | **دفعة مستقلة لاحقة، لا تُنفَّذ الآن.** `PageHero` المشترك يأخذ ارتفاعه من المحتوى حين لا توجد صورة (ADR-0067 D2) لتجنب نحو 550px من السجل الفارغ في الصفحات النصية | تعديل ADR-0067 D2، وتصميم لما يملأ الشاشة في الصفحات بلا صورة |
| **الـ Breadcrumb أعلى الـ hero، على جهة القراءة للغة** | صف مستقل في أول الـ hero: يمينًا في العربي، ويسارًا في الإنجليزي. العنوان والصورة تحته على جهتيهما الثابتتين. مطبَّق في صفحة كلمة الرئيس الآن | لبقية الـ heroes: ضمن دفعة الارتفاع اللاحقة نفسها، لأن موضع المسار يتغيّر فقط حين يملأ الـ hero الشاشة |
| **مسار التنقّل فوق صورة: تباين فاشل كشفه فحص الاتجاه ج** | لون السجل الأسود الخافت `#BDBCB6` فوق الـ scrim عند أفتح نقطة فيه (64% أسود فوق صورة بيضاء، الأرضية `#5C5C5C`) يقيس **3.51:1**، وعند 70% يقيس **4.44:1**، والحد لنص 13px هو 4.5:1. أُضيف `overPhoto` إلى `Breadcrumb`: الآباء بنص الأرضية عند 85%، أي معالجة العنوان الفرعي فوق الصورة، ويقيس **5.41:1** في أسوأ حالة. مطبَّق في صفحة كلمة الرئيس | **FOLLOW-UP:** `PageHero` يمرّر `register="black"` فوق الصورة ولا يمرّر `overPhoto`، فالعيب كامن في كل صفحة يُرفع لها صورة hero. يُعالَج في دفعة الـ heroes اللاحقة |
| **مسار صفحة المراجعة: `/review/president-hero/{a\|b\|c}` بدل `/_review`** | الطريقة الموثَّقة في Next لشريحة تبدأ بـ `_` هي مجلد `%5Freview`. في Next 16.2.12 مع Turbopack، عمل هذا المجلد فقط والذاكرة المؤقتة `.next/dev` دافئة. بعد مسحها لم يُسجَّل أي مسار تحت `[locale]` (الـ manifest بلا مسارات)، فأعاد الموقع كله 404. إعادة تسمية المجلد وتشغيل بارد أعادا كل المسارات | الصفحة داخلية: `noindex`، وخارج التنقّل والـ sitemap و`PUBLIC_PAGES`، ومستثناة بالاسم في `seo-contract.spec.ts` |
| **الـ Breadcrumbs: روابط ظاهرة والعنصر الحالي مميز** | في مكوّن `Breadcrumb` المشترك، فيشمل الصفحات الاثنتي عشرة: الرابط مسطَّر دائمًا لا عند المرور فقط، والعنصر الحالي بلون النص الأساسي للأرضية. «عن الاتحاد» يبقى نصًا لأنه بلا صفحة (IA §8.5) | — |
| **العربي تحت `lg`: الخطوط على مستوى نص الـ hero** (2026-09-13) | يُلغى الحيّز فوق العنوان، وتنزل المجموعة أ إلى مستوى العنوان. مقيس بمحاكاة هاتف حقيقي: الـ hero 571/571 على 375×667، و544/544 على 360×640، و748/748 على 390×844، والمسافة ساكنة 46.8 و37.2 و50.5، وأثناء الدخول 32.9. الخط الأخضر 72px على عرض 360 لتصل الصورة إلى 249px (المسافة 49.2 ساكنة و32.0 أثناء الدخول). يسقط استثناء §5.10 في العربي من أصله | لا نقطة توقف بالارتفاع ولا توثيق استثناء. سطح المكتب بلا تغيير. يُوثَّق في ADR-0069 D10 |

### الـ hero الإنجليزي على الهواتف القصيرة — مؤجَّل بقرار المالك (2026-09-13)

**النتيجة اليوم:** العربي والإنجليزي يختلفان على الموبايل. الإنجليزي تحت `lg` يبقى بالحيّز فوق العنوان كما هو، ويبقى فيه استثناء §5.10 (الـ hero 683px على 375×667 مقابل 571px متاحة و600px سقف 90vh). ثمن مؤقت قبله المالك. لا عكس للمجموعتين، ولا تغيير لسيناريو الدخول، ولا تضييق لعمود العنوان.

**الحالات الثلاث المقيسة** (محاكاة هاتف عبر CDP، الصورة محدودة بـ 249px، المسافة من حافة الخط المرسومة إلى مستطيلات النص والصورة):

| الحالة | الشاشة | الـ hero / المتاح | المسافة ساكنة / أثناء الدخول | الحكم |
|---|---|---|---|---|
| (أ) عكس المجموعة أ إلى اليمين، خط أخضر 48px | 375×667 | 571 / 571 | 34.4 / **23.3** | **مرفوض بصيغته الحالية:** 23.3px أثناء الدخول تكسر قاعدة 32px في D10 (IL-5) |
| | 360×640 | 544 / 544 | **19.4 / 12.8** | |
| | 390×844 | 748 / 748 | 47.9 / 27.4 (المجموعة ب والصورة) | |
| (ب) الحيّز فوق العنوان، كما هو اليوم | 375×667 | **683** / 571 | ≥ 32 | **المعتمد مؤقتًا**، والاستثناء قائم |
| (ج) عكس المجموعة أ مع تضييق عمود العنوان | 375×667 | **586** / 571 (+15) | 50.5 / 0 | **مرفوض:** لا يحقق الهدف، يبقى أطول من الشاشة |
| | 360×640 | **602** / 544 (+58) | — | |

صيغة من (أ) بدخول «ينمو في مكانه» بدل الانزلاق تجعل مسافة الدخول تساوي المسافة ساكنة، لكنها تسقط أيضًا على 360×640 (19.4px ساكنة).

**السؤال المفتوح:** لماذا تُعكس المجموعتان في الإنجليزي أصلًا، وD10 (IL-7) تنصّ على أن الموضع فيزيائي ثابت لا ينعكس بين اللغتين؟ هل ينجح «مستوى النص» في الإنجليزي لو بقيت المجموعة أ على اليسار كما في العربي؟
- قيس بعد طرح السؤال، في الجلسة نفسها: بلا تعديل تقع المجموعة أ فوق الاسم (0px) في المقاسات الثلاثة، لأن النص الإنجليزي يبدأ من الحافة اليسرى. إبعاد العنوان عنها (140px بالخط الحالي، و112px بخط 72px، و85px بخط 48px) يلفّ العنوان إلى سطرين، فيصير الـ hero 586px على 375×667 و586–602px على 360×640، وهي كلفة (ج) نفسها. على 390×844 ينجح (748/748، والمسافة 47.9 ساكنة و32.7 أثناء الدخول).
- القرار بين هذه الصيغ للمالك.

**معادلة حجز مسافة الأمان تحتاج تصحيحًا.** `TITLE_RESERVE` و`PORTRAIT_INSET` مكتوبتان بافتراض مقاس الخط الحالي. عند تصغير الخط تنخفض المسافة أثناء الدخول تحت 32px: 32.0 عند 72px، و30.2 عند 56px، و27.4 عند 48px، و26.4 عند 40px، ومنها 31.7 و27.4 بين المجموعة ب والصورة. تُصحَّح ضمن إصلاح الحارس في المرحلة د، بعد أن يسقطها الحارس أحمر.

### ما بُني في الدفعة الأخيرة — 2026-09-13

- **المسار:** `apps/web/src/app/[locale]/about/president/page.tsx`. يقرأ `GET /president-message-page/current/public`. دون رسالة Live يعيد صفحة 404 المصمَّمة، ويبقى خارج الفهرس والـ sitemap. مسجَّل في `PUBLIC_PAGES` (`president-message`، سجل `green`، `AboutPage`)، وفي `indexability.ts`.
- **معرض المحتوى الغني:** `components/rich-text/rich-text.tsx` على السيرفر، عقدة بعقدة، بلا `dangerouslySetInnerHTML`. يرسم ما تسمح به الـ allowlist فقط، ويُسقط غيره. لا italic في العربي. الروابط بالمخططات المسموحة فقط. الـ allowlist في الـ API لم تُمَسّ.
- **المكوّنات:** `components/pages/president/`:
  - `president-hero.tsx`: التركيب مشتق من البيانات، ج مع خلفية وب بدونها.
  - `president-message.tsx`: الاقتباس والنص والتوقيع في `<article>`.
  - `values-band.tsx`.
  - `reveal-once.tsx`: الظهور مرة واحدة.
- **الأيقونات:** `lib/icons/value-icons.tsx`، نسخة من رسومات الداش بورد، ويحرسها اختبار يقارنها بـ enum الـ API.
- **SEO:** العنوان والوصف من `seo` ثم `heroTitle` و`pullQuote`. `og:image` من `seo.ogImage` ثم الصورة، ويُضاف محليًا في الصفحة، فلا تعديل على `buildMetadata` المشترك. JSON-LD من نوعي `AboutPage` و`BreadcrumbList`.
- **القياس (2026-09-14):**
  - **الحارس:** `e2e/identity-lines.spec.ts`، 24 حالة خضراء. أدنى مسافة 35.3px في العربي على 360×640، و37.0px في الإنجليزي على 640×960، ولا تجاوز أفقي في أي إطار.
  - **CLS وLCP:** `e2e/vitals.spec.ts`، 16 حالة خضراء. انزياح محتوى الصفحة صفر، والصور مؤخَّرة 1.5 ثانية والمعالج مبطَّأ 4×. عنصر الـ LCP هو الصورة في كل تشغيل.
  - **عيبان كُشفا وعولجا:** الصورة بلا مساحة محجوزة قبل تحميلها (0.107)، والـ hero حاوية تمرير (0.0084).
- **صفحة المراجعة:** حُذف مسارها ومكوّنها وملف CSS الخاص بها، وحُذف استثناءاهما من `seo-contract.spec.ts`.
- **فحوص المتصفح:** `apps/web/playwright.config.ts`، و`e2e/identity-lines.spec.ts` (الحارس)، و`e2e/president-text.spec.ts` (المقارنة الحرفية). الـ workflow في `.github/workflows/web-browser-checks.yml` مع خادم API بديل (`e2e/fixture-api.mjs`)، ويشغّله المالك بعد الدفع.
- **البنود الأربعة المفتوحة:**
  - **فراغ 525px فوق العنوان على 1440×900:** من `lg` تلي كتلة العنوان حيّز المجموعة أ مباشرة، كما يلي العنوانُ مسارَ التنقّل في إطارَي Figma `1219:2300` و`1268:2321`. الهامش السفلي لعمود العنوان عند `lg` صار `space-8` بدل `space-20`، فلا يتجاوز الـ hero الشاشة على 1366×657. التضحية: «خط الأساس الواحد» للعنوان والصورة على سطح المكتب في §7.4.
  - **سقف ارتفاع الصورة على 1366×657:** الطول الأقصى من `lg` هو الشاشة ناقص الهيدر وصف المسار (هامشه العلوي، وسطر caption، وهامشه السفلي) وهامش الشبكة العلوي، وكلها tokens.
  - **دون صورة:** عمود واحد في اللغتين، فلا يُحبس العنوان في النصف الأيمن.
  - **عمود العنوان على 1024:** مسار الصورة حدّه 5/12 من الصف من `lg`.
...
| س١٣ | ظهور الفقرات: `rise-scroll` (scroll-driven، بلا JS) أم مرة واحدة بـ IntersectionObserver؟ | ~~`rise-scroll`~~ ← **مرة واحدة بمراقب**، والمحتوى كامل دون JavaScript بلا استثناء (قرار المالك 2026-09-13، ADR-0069 D10) |
...
# تشغيل محلي — سياسات النشر لا تأتي من البذر

**سُجّل 2026-09-12، أثناء الخطوة ٧.**

`npm run seed:dev` لا يبذر مجموعة `workflowPolicies` إطلاقًا: قائمة
`DEV_FIXTURE_SETS` في `api/src/bootstrap/seed-dev.ts` تعدّ سبع عشرة مجموعة،
وليست هذه منها، ولا يبذرها `runBootstrap` (يبذر كتالوج الصلاحيات والمستخدم
الإداري وحدهما). ولا توجد هجرة تنشئها.

وأثر ذلك ليس «الوضع الافتراضي يتطلب موافقات»، بل **الحجب الكامل**:
`WorkflowPoliciesService.resolve()` يفشل مغلقًا عمدًا — غياب الصف يعني
`mode: 'blocked'` و`reason: 'noPolicy'`، فيرفض النشر المباشر **والإرسال
للمراجعة** معًا، كلاهما برمز `publishingPolicyMissing`. الحفظ وحده يظل يعمل،
لأن `assertCanEdit` لا يستشير السياسة.

**فأي جهاز جديد — وأي قاعدة نظيفة — يحتاج ضبط السياسة يدويًا قبل تجربة
النشر.** هذا سلوك مقصود ومغطّى باختبار
(`president-message-publishing.e2e-spec.ts`: «يرفض بـ`publishingPolicyMissing`
حين لا توجد سياسة — ولا يتراجع أبدًا إلى النشر»)، **وليس عطلًا**. وخُضرة
الاختبارات لا تقول شيئًا عن حالة القاعدة المحلية، لأن كل اختبار يضبط سياسته
بنفسه ثم يمسحها.

الضبط يتم عبر `PUT /api/v1/workflow-policies/:entityType/:operation` — المسار
الموجود — لا عبر إضافة السياسات إلى البذر، لأن البذر يشترك فيه كل من يشغّله،
وسياسة النشر قرار تشغيلي يخص كل بيئة على حدة.

الحالة المضبوطة محليًا الآن: `presidentMessagePage / Edit` بـ
`workflowRequired: false` (نشر مباشر لمن يملك `presidentMessagePage:Publish`).

---

# بند مطلوب قبل ٢ أكتوبر — `policy-settings.tsx`

**قرار المالك 2026-09-12، عند اعتماد الخطوة ٧.**

واجهة إعداد النشر الصغرى (`components/admin/editorial/policy-settings.tsx`) أُجّلت
عن الخطوة ٧ لأنها ليست ضمن بنودها، **ويجب أن تدخل قبل ٢ أكتوبر**.

**السبب:** بدونها لا يستطيع مسؤول المنصة تبديل `workflowRequired` — أي التحوّل بين
النشر المباشر ومسار الموافقات — إلا باستدعاء
`PUT /api/v1/workflow-policies/:entityType/:operation` يدويًا برمز وصول. وهذا يعني
أن قرارًا تشغيليًا يخصّ كل بيئة على حدة لا سبيل إليه من داخل المنتج.

الأساس موجود بالفعل: المسار مبنيّ ومختبَر، و`GET /workflow-policies` و
`GET /workflow-policies/:entityType/:operation` يقرآن الحالة، وصلاحيات
`workflowPolicies:Read/Update` في الكتالوج. الناقص هو الواجهة وحدها.

ملاحظة على النطاق: عند `workflowRequired: true` تحتاج الواجهة أيضًا إلى اختيار
`workflowDefinitionId` من التعريفات الفعّالة لنفس `entityType` — وإلا خُزّنت سياسة
يرفضها `resolve()` بـ`definitionMissing`. الـ API يرفض ذلك أصلًا عند الحفظ
(`assertDefinitionUsable`)، فالواجهة تعرض الخيارات الصالحة لا أكثر.

---

# سجل تجريبي للتدريب على النشر — `?record=<id>`

**قرار المالك 2026-09-12، عند اعتماد الخطوة ٨ (البديل أ).**

السجل الحقيقي يحمل علامتَي `[[pending-content]]` مقصودتين، فلا يُنشر ولا تُجمَّد له
إصدارات. ولتجربة النشر المباشر والموافقات والاسترجاع دون المساس به، يُنشأ **سجل
تجريبي منفصل** في نفس المجموعة.

ما يجعله آمنًا ثلاثة أمور، كلها في الكود لا في الانضباط:

1. **لا يظهر في الموقع العام.** `getCurrentPublic()` يقرأ فقط السجلات المرتبطة
   بتعيين `roleType: 'President'` و`status: 'Active'`. فالسجل التجريبي يُربط بتعيين
   `status: 'Completed'`، ويبقى خارج الاستعلام مهما نُشر.
2. **لا يزيح السجل الحقيقي عن الشاشة.** الشاشة تفتح **أقدم** سجل بـ`createdAt` حين
   لا يسمّي الرابط غيره — لا «أول ما أعادته القاعدة»، لأن الترتيب الطبيعي في
   MongoDB ليس عقدًا.
3. **لا يُفتح إلا بطلب صريح.** `/{locale}/president-message?record=<id>`، والمُعرّف
   يُطابَق على القائمة التي أعادها الـ API لهذا القارئ نفسه — لا يُجلب مستقلًا. ورابط
   بمُعرّف غير موجود يعرض **رفضًا صريحًا**، ولا يفتح سجلًا آخر بديلًا: محرّر يظن أنه
   على سجل تجريبي وهو على الحقيقي سينشر واجهة الاتحاد.

الحذف: `DELETE /api/v1/president-message-page/:id` حذف ناعم (`archivedAt`)،
و`find()` يستثني المؤرشف — فالسجل التجريبي يختفي من القائمة ومن الشاشة تمامًا.

---

# البنود المعلّقة عند إغلاق المرحلة ج

ثلاثة بنود خرجت من نطاق المرحلة ج بقرار صريح، ولم يُنسَ أيٌّ منها. مسجَّلة هنا معًا
لأن من يفتح الملف بعد شهر يحتاج قائمة واحدة لا ثلاثة اكتشافات متفرقة.

## ١. `policy-settings.tsx` — **مطلوب قبل ٢ أكتوبر**

واجهة تبديل `workflowRequired` بين النشر المباشر ومسار الموافقات. تفصيلها في قسم
«بند مطلوب قبل ٢ أكتوبر» أعلاه. بدونها لا سبيل إلى قرار تشغيلي يخصّ كل بيئة إلا
باستدعاء `PUT /api/v1/workflow-policies/:entityType/:operation` يدويًا برمز وصول.

## ٢. صياغة `assign_serviceUnavailable` وأخواتها — بند صياغة مستقل

**قرار المالك 2026-09-12، عند اعتماد الخطوة ٩.**

المفاتيح `assign_serviceUnavailable` و`status_serviceUnavailable` و
`save_serviceUnavailable` و`WriteErrors.serviceUnavailable` كلها تقول **«تعذّر
الوصول إلى الخادم»** — وهي الرسالة التي تظهر أيضًا حين يكون السبب ٤٠٤ محليًّا أو
مسارًا غير مسجَّل في الـ BFF. فتوجّه القارئ إلى الشبكة بينما العطل في التوجيه.

**لماذا لم تدخل الخطوة ٩:** تلك الخطوة ترحّل **نتائج الإجراءات** إلى الـ toasts،
وهذه **صياغة خطأ** تبقى inline. تغييرها داخل خطوة الترحيل كان سيخلط تغييرين
مختلفين في تحقق واحد.

**ما تحتاجه:** قرار صياغة — هل يُفرَّق بين «الخادم لا يستجيب» و«الطلب لم يصل إلى
مسار معروف»، أم تُوحَّد الرسالة بصيغة لا تدّعي معرفة السبب. القرار لغوي بقدر ما هو
تقني، ولذلك لم أختره وحدي.

## ٣. المحتوى المطلوب من العميل — **لا يمنع النشر**

**الوضع منذ 2026-09-13:** حذف المالك علامتَي `[[pending-content]]` من شاشة الإدارة
ونُشر السجل. الحذف أزال كل علامة والمسافة التي قبلها فقط، دون أي نص (مقارنة
حرفية بين المسودة والنسخة الحية والـ API والمصدر المعتمد §3). **قرار المالك:** يُنشر
الاقتباس الإنجليزي والفقرة الخامسة بوضعهما الحالي، على أن يُطلب من العميل ما يلي.
الطلب يُرسَل في اليوم نفسه، لأن النص غير المعتمد بلا علامة تحذيرية يمرّ ويُنسى.

آلية العلامة نفسها (`pending-content.ts` و`findPublishBlockers`) باقية في الكود
للمستقبل، لكنها لم تعد تمنع نشر هذا السجل.

| # | البند | الصيغة المطلوبة من العميل | ما يُعرض حتى وصوله |
|---|---|---|---|
| ١ | الاقتباس الإنجليزي | «الاقتباس الإنجليزي المعروض ترجمة مقترحة غير معتمدة». المطلوب: الاقتباس الإنجليزي المعتمد (PM-D04) | ترجمة إطار سطح المكتب المقترحة |
| ٢ | الفقرة الخامسة الإنجليزية | «الفقرة الخامسة الإنجليزية ناقصة جملة، والنقص في الترجمة المعتمدة نفسها (PM-D10)». المطلوب: ترجمة الجملة | النص المعتمد بنقصه |
| ٣ | صورة الرئيس | الصورة الأصلية بلا علامة مائية | الصورة الحالية بعلامتها، دون أي تعديل بالكود |
| ٤ | صورة خلفية لألعاب القوى | صورة معتمدة للخلفية | الصفحة على التركيب ب (بلا خلفية) |
| ٥ | النص البديل الإنجليزي لصورة الرئيس | مكتوب بالعربية («محمد المر»). المطلوب: نص إنجليزي | كما هو |
| ٦ | العنوان الفرعي العربي | يكرر العنوان («كلمة رئيس اتحاد الإمارات لألعاب القوى» تحت «كلمة الرئيس»). المطلوب: صياغة معتمدة | كما هو |

**للتجربة دون انتظار العميل:** السجل التجريبي و`?record=<id>` — القسم السابق.

---

# دفعة الإغلاق النهائية — قرارات المالك 2026-09-13

عند اعتماد المرحلة ج. ما يلي سجلّ قرارات، لا قائمة رغبات: كل بند فيه ما قرّره المالك
ومتى يُفتح.
...
| البند | ما تغيّر |
|---|---|
| **أ. شاشة المستخدمين تُتلف تعديلات الأدوار غير المحفوظة** | كل نموذج فرعي يُشير إلى اكتماله هو فقط: تطبيق الحالة لا يغلق اللوحة، و«إلغاء» في نموذج الأدوار يعيد اختياراته هو ولا يغلق شيئًا. إغلاق اللوحة وفتح غيرها مرفوضان ما دامت هناك تغييرات، موصوفَين بسطر `assignDiff` القائم. **ومسار خامس وُجد أثناء الإصلاح:** مرشِّح يُخفي الصف كان يُزيل اللوحة من الشاشة — الصف قيد التعديل يبقى الآن ظاهرًا ما دامت تغييراته غير محفوظة |
| **ب. إيقاف الحساب بلا تأكيد** | `ConfirmDialog` بصيغة أرشفة الدور للإيقاف والتعطيل وحدهما؛ إعادة التفعيل لا تُنهي شيئًا فتُطبَّق مباشرة. **زر التأكيد يسمّي الفعل لا فئته** بقرار المالك 2026-09-13: «أوقف الحساب» / Suspend account و«عطّل الحساب» / Deactivate account بدل «غيّر الحالة» — سابقة «أرشف الدور» قاعدة لا استثناء. زر «تطبيق الحالة» صار `Button` بحالة loading وارتفاع ٤٤px. تعليق الملف صُحِّح ليصف ما هو قائم |
| **ج. لوحة تأكيد الاسترجاع لا تأخذ التركيز** | التركيز ينتقل إلى «إلغاء» عند ظهورها — آلية `showModal()` وقاعدتها: أول عنصر قابل للتركيز هو الخيار الآمن. و`aria-describedby` على نصّ العاقبة. **وثغرة فقدان بيانات وُجدت أثناء الإصلاح:** حارس التعديلات غير المحفوظة كان يُفحص عند *طلب* الاسترجاع فقط، واللوحة في صفحة حيّة؛ كتابةٌ في النموذج بعد ظهورها ثم ضغط التأكيد كانت تُسترجَع فوقها. يُعاد الفحص عند الضغط. **ثم صار التأكيد `ConfirmDialog` بقرار المالك 2026-09-13** — القسم «الاسترجاع» أدناه |
| **شعار الاتحاد** | في خانة العلامة بدل «لوحة التحكم» — الأصل متاح (`public/brand/`، مطابق بايتًا لـ`docs/design-system/brand-assets` وبالألوان الرسمية). مكوّن واحد `BrandMark` للإطار المسجَّل وغير المسجَّل، يتبع `data-theme` لا الكوكي: مفتاح السمة لا يُحدِّث الخادم، وشعار يُختار مرة من الكوكي كان يبقى بحروفه السوداء على خلفية داكنة — وهو عيب كان قائمًا في صفحة الدخول. المفتاح `Shell.brand` حُذف لأنه صار بلا مستخدم |

## دفعات مستقلة مجدولة

### ١. أخطاء الأنواع في ملفات اختبار الـAPI — **أُغلق 2026-09-13**

**النتيجة:**
- **التقدير سبق الإصلاح:** الأشكال الثلاثة أدناه تغطي ١٣ خطأ فقط من ١٠٧ (في ملفين)، والبقية
  ١٢ شكلًا آخر. طُبِّقت الإصلاحات أولًا في الذاكرة عبر مترجم TypeScript نفسه (١٠٧ ← صفر)
  قبل لمس أي ملف.
- **`tsc --noEmit -p tsconfig.json`:** ١٢٨ ← ١٠٧ بعد `include`/`exclude` (الـ٢١ في
  `documentation/` خرجت، ولم يظهر خطأ لم يُحصَ) ← **صفر** على `src` و`test`.
- **٤١ ملفًا:** ٤٠ ملف اختبار و`test/utils/mongo-memory-server.ts`. لا ملف إنتاجي.
- **الإصلاح المركزي الوحيد:** `registerTestModel`، ويغطي ٣١ خطأ في ١٥ ملفًا. جذره في الكود
  الإنتاجي — البند ٣.
- **الاختبارات قبل/بعد متطابقة:** الوحدات ٨٢٢/٨٢٢ في ٩٢ suite، والـe2e ‏٧٨/٧٨ في ١٦. لم يتغيّر
  سطر `it` أو `describe` أو `beforeAll`، ولا عدد الاختبارات في أي ملف.
- **الـCI:** خطوة `Type-check the API with its tests` في `api-checks`، بـ`--incremental false`
  كي لا يُكتب tsbuildinfo داخل `dist/`. الـ hook لم يتغيّر — `docs/engineering/quality-gates.md`.
- **انحراف في الاختبارات كشفه الفحص، لا عيب منتج:** مسار استيراد غير موجود في
  `documents.service.spec` (كان يعمل لأن الاستيراد يُحذف عند الترجمة)، وبيانات تسبق حقلي
  `photographer`/`captureDate` (قيمتهما الافتراضية `null`، فالوثيقة المحفوظة لم تتغيّر)، و
  `MediaAssetsService` يُبنى بمعاملين بعد إضافة `storage`.

**كما سُجِّل عند الجدولة:**

**القرار:** `api/tsconfig.json` يأخذ `include: ["src", "test"]` و`exclude: ["documentation"]`،
ثم تُنظَّف الأخطاء دفعةً، ثم يُضاف `tsc --noEmit` على الاختبارات إلى الـCI.

**لماذا الآن لا لاحقًا:** العدد يكبر مع كل خطوة (٩٧ ← ١٠٧)، و`tsc` أمسك في المرحلة ج ما لم
يمسكه أي اختبار (`setOutcome` في زر الاستعادة). ملفات اختبار الـAPI لم تُفحص أنواعها
قطّ: `nest build` يستثني `**/*spec.ts` و`test/`، و`ts-jest` لا يفحص.

**عند فتحها:** يُقدَّم تقدير هل تُحلّ الأشكال الثلاثة المتكررة بإصلاح واحد لكل شكل لا ملفًا
ملفًا:
- `Namespace 'supertest' has no exported member 'default'`
- `No overload matches this call` عند `model.create({...})`
- `Property '_id' does not exist on type 'never'` — متتالٍ عن السابق

### ٢. أرضية الـ44px — قبل ٦ نوفمبر

أزرار مكتوبة يدويًا خارج `interactive.ts`، بارتفاع ثابت دون `TOUCH_TARGET` (`min-h-11`)
الذي يعرّفه `interactive.ts` نفسه (Protocol §14)، وبحلقة `outline` التي استبدلها ADR-0051.
الإصلاح الجذري موجود: `Button` وثوابت `interactive.ts`. **تغيير بصري**، ولذلك دفعة مستقلة.

**٢٤ موضعًا؛ واحد نُفِّذ في هذه الدفعة بقرار المالك.** مسجَّلة بالملف ومفتاح النص لأن أرقام
الأسطر تتحرك؛ الأرقام أدناه كما هي في 2026-09-13 للإرشاد فقط.

| الشاشة | الملف | الزر (مفتاح النص) | الارتفاع | الحالة |
|---|---|---|---|---|
| المستخدمون | `users/status-control.tsx` | `applyStatus` | كان h-10 | **نُفِّذ** |
| المستخدمون | `users/user-directory.tsx:133` | `newUser` / `closeEditor` | h-10 | |
| المستخدمون | `users/user-directory.tsx:225` | `editAccess` / `closeEditor` (زر الصف) | h-9 | |
| المستخدمون | `users/create-user-form.tsx:253` | `createSubmit` | h-10 | |
| المستخدمون | `users/create-user-form.tsx:260` | `formCancel` | h-10 | |
| المستخدمون | `users/role-assignment.tsx:172` | `cancel` | h-10 | |
| المستخدمون | `users/role-assignment.tsx:186` | `saveRoles` | h-10 | |
| الأدوار | `roles/role-editor.tsx:161` | `createSubmit` / `saveDetails` | h-10 | |
| الأدوار | `roles/role-editor.tsx:168` | `formCancel` | h-10 | |
| الأدوار | `roles/role-workbench.tsx:244` | `newRole` | h-9 | |
| الأدوار | `roles/role-workbench.tsx:310` | `editDetails` | h-9 | |
| الأدوار | `roles/role-workbench.tsx:317` | `archiveRole` | h-9 | |
| الأدوار | `roles/role-workbench.tsx:358` | `lens_*` (مقطع العدسة) | h-9 | |
| الأدوار | `roles/role-workbench.tsx:403` | `formCancel` (لوحة الأرشفة) | h-10 | |
| الأدوار | `roles/role-workbench.tsx:472` | `onlyGranted` | h-10 | |
| الأدوار | `roles/role-workbench.tsx:542` | `reset` | h-10 | |
| الأدوار | `roles/role-workbench.tsx:550` | `save` | h-10 | |
| الصفحات | `pages/page-editor.tsx:207` | `reset` | h-10 | |
| الصفحات | `pages/page-editor.tsx:218` | `save` | h-10 | |
| الصفحات | `pages/page-editor.tsx:528` | `removeRow` | h-9 | |
| الصفحات | `pages/page-editor.tsx:541` | زر الإضافة (`addLabel`) | h-10 | |
| الصفحات | `pages/media-picker.tsx:77` | `chooseImage` | h-10 | |
| الصفحات | `pages/media-picker.tsx:87` | `noImage` | h-10 | |
| الصفحات | `pages/media-picker.tsx:361` | `uploadSubmit` | h-10 | |

عند فتحها: قاعدة في `interaction-state-contract.spec.ts` تجعل الأرضية غير قابلة للكسر —
عنصر يحمل `--button-primary-background` أو `--button-radius` يجب أن يُحلّ إلى ثابت مسجَّل،
و`focus-visible:outline-` مرفوضة خارج السجلّ. الآلية التي تحلّ الأسماء موجودة في الملف.

### ٣. `Model<XDocument>` في ٦٦ repository — مسجَّل، **لا يُنفَّذ الآن**

**قرار المالك 2026-09-13، عند اعتماد دفعة الأنواع.** غير مجدول.

**ما هو:** ٦٦ ملف repository في الإنتاج (قياس 2026-09-13) تستقبل النموذج بنوع
`Model<XDocument>`، حيث `XDocument = HydratedDocument<X>` — مثل `UsersRepository` و
`PermissionsRepository`. هذا النمط لا يتوافق مع أنواع Mongoose 9: `model()` لا يُنتج
`Model<HydratedDocument<X>>` من `Schema<X>`، و`Model<X>` لا يُسنَد إليه.

**لماذا لا يظهر في التطبيق:** Nest يحقن النموذج عبر `@InjectModel` بلا فحص نوع، فالتطبيق
يعمل ولا يمرّ المترجم على هذا الإسناد قط. يظهر فقط حين يبني الاختبار النموذج بنفسه — وكان
٣١ من أخطاء دفعة الأنواع الـ١٠٧، في ١٥ ملفًا.

**الحصر ريثما يُحسم:** `registerTestModel` في `api/test/utils/mongo-memory-server.ts` تحمل
الـ cast الوحيد الذي يعبر الفجوة. الاختبارات تستدعيها ولا تنسخ الـ cast، فإن أُصلح الجذر
كان التنظيف في موضع واحد لا في خمسة عشر.

**الإصلاح الجذري في الكود الإنتاجي**، ولذلك خرج من دفعة الأنواع التي منعت أي تعديل عليه.

## قرارات تصميم مطلوبة

### المراجِع الذي يملك `Approve` فقط — تُحسم بعد المرحلة د

**المشكلة:** الشاشة تُفتح لمن يملك `workflowInstances:Approve` بلا `presidentMessagePage:Update`،
وهذا صحيح. لكن ما يقرؤه هذا المراجع هو **حقول نموذج معطَّلة** — نحو عشرين حقلًا رماديًا
بجوار عمود قرار بعرض 340px. لا عرض قراءة للنص الذي يُعتمَد. تركيبة صلاحيات قائمة
بلا شاشة خلفها.

**أصغر حل دون تركيبة جديدة:** إعادة استخدام `RevisionReader` — المكوّن القائم الذي يعرض
محتوى سجلّ نصًّا في `<dl>`، بصفَّي العربية والإنجليزية لكل حقل ثنائي اللغة — لعرض **المسودة
الحالية** بدل النموذج المعطَّل حين لا يملك القارئ `Update`. لا مكوّن جديد، ولا نمط جديد:
نفس القارئ الذي يقرأ به المحرّر إصدارًا قديمًا.

**المقايضة:** `RevisionReader` يُخرج النص الغني نصًّا بلا تنسيق — عمدًا، كي لا يمرّ ترميز
نسخة قديمة خارج allowlist المحرّر. فمراجع يعتمد **التنسيق** (خط عريض، قوائم، روابط) لن
يراه. البديل الذي يحفظ التنسيق: محرّر TipTap نفسه بـ`editable: false` — لكنه يبقى يُقرأ
كنموذج. القرار: هل يعتمد المراجع النص أم النص وتنسيقه؟

### الاسترجاع: لوحة في الصفحة أم `ConfirmDialog` — **حُسم: `ConfirmDialog`، ونُفِّذ (2026-09-13)**

قرار «لوحة في الصفحة» بُني على أن الإصدار المعني يبقى ظاهرًا أثناء السؤال. ما ظهر بعد البناء:

| | لوحة في الصفحة (القائم) | `ConfirmDialog` |
|---|---|---|
| الإصدار المعني ظاهر أثناء السؤال | اللوحة تُرسم **بعد** قائمة الإصدارات (حتى ٢٠ صفًا)، ونقل التركيز إليها يمرّر الصفحة، فيخرج الصف المعني من الشاشة في الغالب | لا — لكن الحوار يسمّي رقم الإصدار وتاريخه نصًّا، وهما ما يحتاجه القرار |
| الصفحة خلف السؤال | **حيّة**: النموذج يقبل الكتابة. هذا ما فتح ثغرة فقدان البيانات، وأُغلقت بإعادة الفحص عند الضغط — شرطٌ يجب ألا يُنسى | **خاملة** (`showModal()`): الثغرة مستحيلة بنيويًا لا مغلقة بشرط |
| التركيز · `Esc` · إعادة التركيز للزر الذي فتحها | التركيز أُضيف في هذه الدفعة؛ `Esc` غير موجود؛ الإلغاء يُزيل اللوحة فيضيع التركيز | كلها من المنصة |
| الاتساق على الشاشة نفسها | آلية ثالثة (النشر والاعتماد بحوار) | نفس آلية النشر والاعتماد |
| التكلفة | لا شيء إضافي | نقل نحو ثلاثين سطرًا، وتحديث اختبارات التأكيد في `revisions-panel.spec.tsx` لتستهدف `dialog` بدل `alertdialog` |

**التوصية:** `ConfirmDialog`. سبب القرار الأصلي لم يصمد أمام التنفيذ — الصف يخرج من الشاشة
على أي حال — بينما صار للحوار ميزة لم تكن محسوبة: الخمول يجعل ثغرة اليوم مستحيلة لا مُغلقة.

**القرار (2026-09-13):** قُبلت التوصية. الحجّة أسقطت أساس القرار الأول — الصف يخرج من
الشاشة على أي حال، فالمكسب المفترض غير موجود — والحوار يغلق الفجوة بنيويًا بدل شرط يجب
ألا يُنسى.

**كيف نُفِّذ:**
- الحوار **مركَّب طوال الوقت**، فالإغلاق هو `close()` ويعود التركيز إلى زر الاسترجاع الذي فتحه.
  ونصّه لا يوجد إلا وهو مفتوح: حوار مغلق يحمل عنوان إصدار جملةٌ في الصفحة لا يراها أحد.
- **يُغلق عند رفض الاسترجاع** كي يُقرأ الرفض: الصفحة خلف الحوار خاملة، ورسالة تُرسم هناك
  والحوار مفتوح لا تُرى ولا تُسمع.
- **إعادة الفحص عند الضغط بقيت.** الحوار يغلق مسار الكتابة؛ ولا يغلق تغيّر السجل تحته.
  لم تعد خط الدفاع الأول، ونسيانها لم يعد يُفقد عملًا.

## ما رُفض في `/simplify` ويُسجَّل

| البند | لماذا لم يُنفَّذ |
|---|---|
| عنقود الأزرار اليدوية (٢٢ موضعًا) | تغيير بصري خارج نطاق المرحلة — صار دفعة الـ44px أعلاه |
| `StatusMessage` بدل ٥ صناديق تنبيه يدوية (`status-panel`، `revisions-panel`، `editor`) | تغيير بصري (شريط النغمة وخلفية مُلوَّنة)، وأحدها يكتسب `role="alert"` — تغيير في المناطق الحيّة |
| مساعد `adminFetch` للمتصفح (١٣ موضعًا تكرّر قراءة جسم الفشل) | إعادة هيكلة واسعة؛ يربط رمز الخطأ على المتصفح بـ`WriteErrorCode` المحروس بالاختبار |
| `errors(failure)` محروسة بـ`errors.has()` في ثلاثة ملفات وغير محروسة في ثلاثة | قاعدة واحدة بجوابين — تُحسم مع `adminFetch` |
| استخراج `RoleCheckboxList` (منسوخة بين `create-user-form` و`role-assignment`) | استخراج مكوّن؛ الانحراف الوحيد بينهما (`disabled:opacity`) أُصلح |
| `DataTable` بلا مستدعٍ واحد، والحالة الفارغة منسوخة ثلاث مرات | حذف أم توسيع ليحمل صفًّا قابلًا للتوسيع — قرار |
| حقول `readPermission`/`updatePermission`/`publishPermission` في `editorial-entities.ts` بلا قارئ | حذفها، أو جعلها فعّالة بـ`canEditEntity(grants, entity)` تستدعيه كل صفحة — قرار قبل الصفحات الإحدى عشرة |
| `readGrants` بالتوازي مع جلب السجل في `president-message/page.tsx` | مقايضة أمنية الشكل: تُجلب بيانات لمن قد لا يراها (يرفضها الـAPI، لكنها طلبات مهدرة) |
| `bothHalvesFilled` في `page-editor.tsx` تكرار لـ`isLocalizedText` في `request-shapes.ts` | قاعدة النموذج وقاعدة المسار قد تفترقان؛ لكن `isLocalized` المحليتين **يجب ألا** تُستبدلا — تُنفَّذ بحذر |
| زر بـ`loading` يبدّل نصه أيضًا (`status-panel`، `revisions-panel`، `editor`) | `Button` يُبقي النص في شجرة الوصول؛ حذف التبديل يغيّر الاسم المنطوق أثناء التنفيذ — قرار وصولية |
| عقدان موثَّقان لـ`fieldLabels` (مسار الحاجز مقابل اسم الحقل المخزَّن) | يتطابقان لكلمة الرئيس؛ أول صفحة يختلفان فيها تُظهر اسمًا في لوحة ومسارًا خامًا في الأخرى — جملة توثيق تحسم أيّهما يغلب |
| ثلاث اصطلاحات لخطأ التحقق المحلي (`__missingFields`، `nameRequired`، لا شيء في المحرّر) | توحيد اصطلاح |
| مساعدات صفوف مكرّرة ست مرات في `page-editor.tsx` | كود أقدم من المرحلة ج |
| تسلسل `JSON.stringify` لكامل مستند النص الغني مع كل ضغطة (`editor.tsx`) | يُقاس قبل تغييره — ضُبط مرة عن قصد |
| إعادة جلب إصدار مجمَّد عند فتحه مرة ثانية | هامشي |

## بند صحّة مستقل — باختباره الخاص

**`useState(images)` في `president-message/editor.tsx` يتجاهل تحديث الـprop.** المبدئ يعمل
عند التركيب فقط، فبعد كل `router.refresh()` تصل مكتبة الوسائط المحدَّثة وتُهمَل: المنتقي
يعرض المكتبة كما كانت عند التركيب مضافًا إليها ما رُفع محليًا. الإصلاح: نمط مزامنة الـprop
المستخدم في `status-panel.tsx` («adjust state when a prop changes»)، مع الحرص على ألا
تُمسح صورة رُفعت محليًا قبل وصول التحديث. يحتاج اختبارًا أحمر خاصًا به.

## ملاحظات مسجَّلة لا تُنفَّذ

- **العمليات المجمَّعة (bulk):** لا توجد في أي شاشة. إنشاء اثني عشر حسابًا يعني اثنتي عشرة لوحة.
- **ثلاث نقاط طيّ لأربع شاشات:** `lg:` في المحرّر (السائدة، بسبب موثَّق)، و`2xl:` في الأدوار
  (بسبب موثَّق: المصفوفة تحتاج ~820px)، و`xl:` في `page-workbench` **بلا سبب مذكور**.
- **`text-caption` مقابل `text-body-sm` لنفس دور الرسالة:** `text-caption` يصير 12px دون
  `md`، ويحمل في شاشة المستخدمين البريد وشارات الأدوار وتحذير إنهاء الجلسات؛ بينما تستخدم
  لوحة الحالة `text-body-sm` لنفس الأدوار. **DESIGN SYSTEM GAP** بين سُلَّم الفصل ٤ §٤.٧
  وأرضية 13px.
- **الجدول الزمني غير مسقوف** في `timeline.tsx`، بينما لوحة الإصدارات بجواره تُرقِّم بعشرين.
  الـAPI يسقف السجل عند ١٠٠.
- **`Intl.ListFormat` بدل الفاصل:** الفاصل صار مفتاحًا في `Common.listSeparator` فالإنجليزية
  صحيحة. `Intl.ListFormat("ar")` يُدخل حرف العطف («أ وب وج») — قرار في نحو القائمة العربية
  لا تنظيف.

## تمريرة مراجعة للشاشات الأقدم — قبل التسليم النهائي في ٦ نوفمبر

شاشات المستخدمين والأدوار والصفحات بُنيت **قبل** أن تستقرّ معايير المرحلة ج: سجلّ
`interactive.ts` و`Button`، و`ConfirmDialog`، والـtoasts، وحرّاس التعديلات غير المحفوظة.
مراجعة المرحلة ج مرّت عليها لأن الترحيل لمسها، فوجدت فيها **فقدان بيانات** (أ) و**عقد وصول
مكسورًا** (ب، والأزرار دون 44px) — لا تحسينات شكلية.

الشاشات الأخرى التي لم يمسّها الترحيل لم تمرّ بهذه المراجعة بعد. **كل شاشة أقدم تحتاج
التمريرة نفسها قبل ٦ نوفمبر:** تقييم `impeccable` بمساريه (مراجعة تصميم + كاشف حتمي)، ثم
الإصلاحات التي هي فقدان بيانات أو عقد وصول مكسور، بدورة اختبار أحمر أولًا.

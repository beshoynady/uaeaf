# ADR-0099: Media Showcase Motion

| Field | Details |
| --- | --- |
| **Status** | Accepted. Recorded 2026-09-25, ahead of the components it governs (ADR-0056 §2 — document-first: the record is written before the change is implemented). |
...
| **Amends** | **Chapter 3 §3.14** — a bounded, component-local exception to the closed duration scale, for cycle periods only (D3). **ADR-0098 D5** — `orbit`'s "exactly two bounded places (hover, live)" gains a third bounded role, *showcase rotation*, under D1's cumulative conditions. **Chapter 8 L6** — two components are registered, discharging ADR-0032's Pure Assembly requirement for the homepage Photo Gallery section (D5). |
...
**Scope fence.** D1 applies to media showcase components — `FeaturedAlbumDeck`, `PhotoStack`, and any future component whose purpose is to present a body of media work. It is **not** a general exception. It does not extend to navigation, to status, to a hero, to a data display, or to the dashboard in any part (ADR-0098 D6's Operational dose prohibition stands).

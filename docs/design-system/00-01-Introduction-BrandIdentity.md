...
> **Partially superseded by ADR-0051** (Chapter 3 §3.35) — `color.semantic.success` no longer aliases `color.brand.primary`; it now resolves to the independent `color.success.*` family defined by the UAEAF Digital UI Brand Guide v1.0. The rest of this ADR (green = the only Primary Brand Action / non-danger CTA color; red never a standard CTA) still stands.
...
> **Partially superseded by ADR-0051** (Chapter 3 §3.35) — the generic `color.semantic.danger`/`error` slot no longer aliases `color.brand.secondary`; it now resolves to the independent `color.error.*` family. `live`/`achievement` (and the general "Secondary Brand Accent" role this ADR defines) are unaffected and still resolve through `color.brand.secondary`.

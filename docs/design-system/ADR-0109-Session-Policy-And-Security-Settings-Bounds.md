...
| **Amends** | `siteSettings.sessionTimeoutMinutes` and `siteSettings.maxLoginAttempts` are **deprecated and migrated** into the new `securitySettings` singleton. `config/auth.config.ts` keeps only the bounds. |

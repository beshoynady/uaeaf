...
|---|---|---|---|
| Personality | frame named "(Editorial)" | Visual Protocol §4 — *Institutional: About / governance / board / committees*; §3.34.2 Board row | Institutional. **Amended by ADR-0069 D8:** the page stays Institutional and everything below the hero keeps the site standard (`rise-scroll`, no opacity on reading text, no parallax on the reading column). The **portrait hero** — a hero built around one human portrait — is a documented composition that adds three hero-only behaviours: an `ambient` background settle (ADR-0069 D9), a clip-reveal on the portrait beginning at 86% height, and the motif drawn along each stroke's own axis after the text. The ascent keeps its 45° (ADR-0059 §D7.1); reading order is carried by sequence, not direction. The H1 gets a transform-only entrance because it becomes the LCP element when no background image is set. |

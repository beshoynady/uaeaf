...
|---|---|---|---|
| Personality | frame named "(Editorial)" | Visual Protocol §4 — *Institutional: About / governance / board / committees*; §3.34.2 Board row | Institutional. **Amended by ADR-0069 D8 and D10:** the page stays Institutional. The **portrait hero** — a hero built around one human portrait — is a documented composition with hero-only behaviours, played once, transform only, and done by 680ms: an `ambient` background settle when a background image exists (ADR-0069 D9), the breadcrumb and title block settling at 45° in reading sequence, a 16px rise of the portrait over `slower`, and the identity lines entering along their own axes (ADR-0069 D10). The ascent keeps its 45° (ADR-0059 §D7.1); reading order is carried by sequence, not direction. The H1 gets a transform-only entrance because it is the LCP candidate when no portrait is set. Below the hero, blocks reveal once as they enter view (ADR-0069 D10, Q13 amended), body paragraphs never move, nothing replays, and there is no opacity on reading text and no parallax on the reading column. |
...
| Hero composition | AR: portrait on the reading edge, text centred on the far side; EN mobile: portrait over the H1; portrait bleeds 140px into the header (PM-D07) | ADR-0066 D5 + ADR-0067 D2 (`PageHero`); **amended by ADR-0069 D10** | Title block on the left and portrait on the right **in both languages**. The president faces the left of the frame, so on the right he faces the text in either language, and a photograph of a real person is never mirrored. The breadcrumb is its own row on the reading side. The two never overlap; at `lg` the portrait takes at most 5 of 12 columns and stands on the hero's bottom edge, while the title block follows group A's reserve directly, as the title follows the breadcrumb in the Figma frames (`1219:2300`, `1268:2321`). A shared baseline at `lg` left 525px of empty green above the title at 1440×900 (amended 2026-09-13; below `lg` the title still sits directly above the portrait). Without a portrait the title takes the whole row in either language. The identity lines' reserves (ADR-0069 D10, IL-5) set the space above the title and the portrait's inset. With a background image: `HERO_SCRIM` over it and the hero owns the first screen. No bleed above the hero. Below `lg` the columns stack **heading first** (Chapter 5 §5.10). Fixes PM-D07, D19, D20, D21, D25. **PENDING FIGMA BACK-SYNC.** |
...
| Decorative swooshes in the Speech Card and Values band | 8 vectors, red among them | ADR-0066 D5 (artwork never behind text), Visual Protocol §18 (one approved motif), §3.34.2 Institutional (red virtually absent) | Not drawn. No motif is drawn on this page: the hero carries the identity lines under ADR-0069 D10, and the Speech Card and Values band stay undecorated. Fixes PM-D18. |
...
1. **RESOLVED — ADR-0069.** Pull-quote size: **`h3`** (24/20, Bold). 22px is not on the Ch.4 §4.4 scale and was not mapped silently (the PB-GAP pattern, CLAUDE.md §7); the owner chose the nearer scale step upward so the quote stays distinct from the 20px `h4` used by value-card titles.
2. **RESOLVED — ADR-0069.** Values band register: **`green`**, the page's own register. Figma's `#0d1f12` matches no register and is not used. A card drawn *on* a coloured register still has no recipe — that gap is now live, and is carried on ADR-0069's PENDING FIGMA BACK-SYNC list rather than closed by assumption.
3. **RESOLVED — ADR-0069.** The width §4.6 frees on desktop: **a single centred reading column.** The editorial pair (reading column plus a margin column carrying the quote and signature) was the alternative; it was not taken, because no Figma frame exists for either and the single column is the one that needs no new component.
4. **RESOLVED — ADR-0069.** Sign-off and date: name and title render from `signatoryName` / `signatoryTitle` (display snapshots, per the schema's decision #4), and the date is **derived from the Live publication's `publishedAt`** in a `<time>` element — no new field. No frame shows either; **PENDING FIGMA BACK-SYNC.**

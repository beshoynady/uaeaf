 ...
 *
 * The same drawings as `apps/web/src/lib/icons/plan-phase-icons.tsx`, so an
 * editor sees in the picker what a visitor sees on the page.
 * `plan-phase-icons.spec.tsx` fails if this set and the API's enum disagree.
 */

/** The API's order, which is the order the picker offers. */
...
/**
 * One glyph, sized by its container, out of the accessibility tree: it
 * repeats a choice the author made in the named select beside it. An unknown
 * key draws nothing rather than throwing, so a key retired from the enum
 * later still leaves the stored phase editable.
 */
export const PlanPhaseIcon = ({ iconKey, className = "size-5" }: { iconKey: string; className?: string }) => {
  if (!isKnown(iconKey)) {
    return null;
  }

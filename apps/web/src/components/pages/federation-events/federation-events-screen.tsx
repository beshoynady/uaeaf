...
/**
 * Federation events, in preparation (`PREPARING_PAGES`), on `@uaeaf/brand-ui`.
 *
 * There is no public read for federation events yet, so there is no next event
 * to feature on the red ground and no event cards to border by state.
 * Inventing either would be federation data nobody published.
 *
 ...
 */
export const FederationEventsScreen = async ({ locale }: { locale: AppLocale }) => {

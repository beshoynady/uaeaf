...
/**
 * Disciplines, on `@uaeaf/brand-ui`: the hero (the record's photograph where it has
 * one, the ink ground where it has none) and an empty state.
 *
 * `disciplines` exposes no `@Public()` read upstream, so there is no directory
 * to list: no cards, no search, no filter chips and no count band. The empty
 * state says what the page is instead of letting it end at the hero.
 */
export const DisciplinesScreen = async ({ locale }: { locale: AppLocale }) => {

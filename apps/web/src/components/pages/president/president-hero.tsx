...
import { HERO_MEASURE, HERO_PARALLAX, HERO_SCRIM, HERO_TEXT, HERO_VIEWPORT } from "@/components/ui/surface";
...
/**
 * The portrait hero of the President's Message (ADR-0069 D8, D10).
 *
 ...
 * (composition C); without one it stands on the page's green register
 * (composition B). An editor changes the composition by uploading or removing
 * the picture, never by a setting.
 *
 * - The portrait on the right and the title block on the left, in both
 *   languages, never overlapping. Side by side from `lg`, where the portrait
 *   takes at most 5 of the 12 columns; below it the title comes first and the
 *   portrait follows (Chapter 5 §5.10). Without a portrait the title takes the
 *   whole row in either language.
 * - At least the screen minus the header; where the title, the lines and the
 *   portrait cannot all fit it grows instead.
...
const IL_VARIABLES = [
  ...
  "max-md:rtl:[--il-unit:min(calc(var(--space-24)/81.2),var(--il-unit-keep-portrait))]",
...
/** Both groups start together at this step, once the role line has begun
 *  (breadcrumb 0, title 1, name 2, role 3); inside a group the strokes follow
 *  the reading order one stagger step apart. */
const LINES_STEP = 5;

const step = (n: number): CSSProperties => ({ "--pm-step": n }) as CSSProperties;

const strokeFill = (tone: Stroke["tone"], onPhoto: boolean): string => {
  if (tone === "red") return "var(--color-brand-secondary)";
  if (tone === "green") return "var(--color-brand-primary)";
  return onPhoto ? "var(--color-text-on-brand)" : "var(--color-brand-black)";
};
...
const LineStroke = ({
  ...
  return (
    <span
      className={`il-stroke absolute ${className}`}
      data-il-group={group}
    ...
    >
      ...
      >
        <path
          d={stroke.ribbon.d}
          fill={strokeFill(stroke.tone, onPhoto)}
...
const GroupA = ({ onPhoto, className, top }: { onPhoto: boolean; className: string; top: string }) =>
 ...
 *  drawn here except in Arabic below `lg`, where it stands beside the title.
 *
...
const IdentityLines = ({ onPhoto }: { onPhoto: boolean }) => (
  <div
    aria-hidden="true"
    data-identity-lines=""
  ...
  >
    <GroupA onPhoto={onPhoto} className="block max-lg:rtl:hidden" top="var(--space-8)" />
    {GROUP_B.map((stroke, i) => (
      <LineStroke
        ...
        onPhoto={onPhoto}
        className="block"
        position={{ right: unit(B_WIDTH - stroke.tail - reach(stroke)), bottom: 0 }}
      />
    ))}
  </div>
);
...
export const PresidentHero = ({
  ...
  const titleId = "president-hero-title";
  ...
  return (
    <section
      ...
      data-register="green"
      className={`relative isolate flex ${HERO_VIEWPORT} w-full flex-col overflow-clip ${IL_VARIABLES} ${
        ...
        >
          <div className={`${HERO_TEXT} flex flex-1 shrink-0 flex-col lg:self-stretch lg:pb-8`}>
            <div
              aria-hidden="true"
              data-il-reserve=""
              className="h-[var(--il-reserve)] shrink-0 max-lg:rtl:h-0"
              style={{ "--il-reserve": TITLE_RESERVE } as CSSProperties}
            />
            <div className="relative mt-auto lg:mt-0">
              {/* Group A beside the title, in Arabic below `lg` only. */}
              <div
                aria-hidden="true"
              ...
              >
                <GroupA onPhoto={onPhoto} className="block" top="0px" />
              </div>
              <h1 id={titleId} className="pm-settle relative text-h1 text-balance" style={step(1)}>
                {record.heroTitle[locale]}
              </h1>
              {/* The name attributes the message; it is not a section of the
                  page, so it takes the h2 size without the h2 element. */}
              <p className="pm-settle relative mt-6 text-h2 text-balance" style={step(2)}>
                {record.signatoryName[locale]}
              </p>
              <p
                className={`pm-settle relative mt-2 ${HERO_MEASURE} text-body-lg ${onPhoto ? "opacity-85" : green.muted}`}
                style={step(3)}
              >

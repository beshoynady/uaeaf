...
import { CoachesScreen } from "@/components/pages/coaches/coaches-screen";
 ...
 *
 * `coaches` exposes no `@Public()` read upstream.
 *
...
const CoachesPage = async ({ params }: { params: Promise<{ locale: AppLocale }> }) => {
  const { locale } = await params;
  ...
  return <CoachesScreen locale={locale} />;
};
